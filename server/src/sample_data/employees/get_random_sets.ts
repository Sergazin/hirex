...
import { Language, LanguageEnum } from "@/ts_server";
...
export function get_random_languages(): Language[] {
  ...
  const languages = Array.from(
    new Set(new Array(qty).fill(0).map(() => languages_list[Math.floor(Math.random() * languages_list.length)])),
  );
...
export function get_random_skills() {
  ...
  const qty = Math.floor(Math.random() * 10) + 3;
